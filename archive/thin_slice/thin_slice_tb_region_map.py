...
def return_data_for_map():
    ...
    df_pop = pd.read_excel(
        ...
        sheet_name="Mid-2022 ICB 2023",
        header=3,
    )
    df_pop_agg = (
        df_pop.groupby(["NSHER 2023 Name", "NHSER 2023 Code"])["Total"]
        .sum()
        .reset_index()
    )
    ...
    joined_df = df.merge(
        df_pop_agg, left_on="region_name", right_on="NSHER 2023 Name", how="left"
    )
...
def create_plt_map(data):
    ...
    data.plot(
        ax=ax, column="births per 1000", cmap=nhs_cmap, edgecolor="black", legend=True
    )
    ax.set_axis_off()
